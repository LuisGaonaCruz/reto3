...
def register():
    try:
        ...
        if request.method == "POST":    
            ...
            content = 'Hola, para activar tu cuenta, por favor haz click en el siguiente link ' + flask.url_for('auth.activate', _external=True) + '?auth=' + number
            
            send_email(credentials, receiver=email, subject='Activate your account', message=content)
            
            flash('Por favor revisa tu correo registrado para activar tu cuenta')
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if user is not None:
                ...
                content = 'Hola, para cambiar tu contraseña, por favor click sobre este link ' + flask.url_for('auth.change', _external=True) + '?auth=' + number
                
                send_email(credentials, receiver=email, subject='New Password', message=content)
                
                flash('Por favor revisa en tu email registrado')
            else:
